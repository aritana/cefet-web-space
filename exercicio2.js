...
const fotoAtualGaleriaEl = document.querySelector('#slide');
const buttonAnteriorEl = document.querySelector('#anterior');
const buttonProximoEl = document.querySelector('#proximo');
let currentImage = 0;


buttonAnteriorEl.addEventListener('click', function() {
    if (currentImage - 1 < 0) {
        currentImage = 6;
    } else {
        currentImage = (--currentImage) % 7;
    }
    nextImage();
});
buttonProximoEl.addEventListener('click', function() {
    currentImage = (++currentImage) % 7;
    nextImage();
});

function nextImage() {
    fotoAtualGaleriaEl.src = servidorDasImagens + '/' + imagens[currentImage].arquivo;
    fotoAtualGaleriaEl.alt = imagens[currentImage].descricao;
}